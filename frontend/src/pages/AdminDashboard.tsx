...
const AdminDashboard: React.FC = () => {
  const { user, getUsers, adminUpdateUser, logout, token } = useAuth();
  ...
  useEffect(() => {
    ...
    init();
  }, [user, navigate, fetchUsers, fetchAdminLogs]);
  ...
  const handleStartEdit = (u: User) => {
    ...
    // City and Barangay will be set once cities/barangays are loaded via effects
    // This is a bit tricky with async data, so we'll just show current names in UI if code not found
  };
    ...
    <div className="app-container">
      <header className="app-header">
        <div className="header-left">
          <h1>City Health DMS (ADMIN)</h1>
        </div>
        <div className="header-right">
          <div className="user-profile">
            <span className="badge badge-danger">ADMIN: {user?.fullName}</span>
          </div>
          <ThemeToggle />
